...
pub fn generate_palette_gradient_bicolor(
    count: usize,
    color_start: Color,
    color_end: Color,
) -> Vec<Color> {
...
pub fn generate_palette_gradient_multiple(count: usize, colors: &[(Color, f64)]) -> Vec<Color> {
    assert!(colors.len() > 1, "There must be at least 2 colors");
    assert_eq!(
        colors.first().unwrap().1,
        0.,
        "First color placement must be 0"
    );
    assert_eq!(
        colors.last().unwrap().1,
        1.,
        "First color placement must be 1"
    );
    let mut palette = Vec::with_capacity(count);
